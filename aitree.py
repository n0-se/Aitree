...
def generate_tree_string(paths):
    ...
    def format_tree(current_node, prefix=""):
        lines = []
        ...
        for i, entry in enumerate(entries):
            ...
            if current_node[entry]:
                ...
                lines.extend(format_tree(current_node[entry], prefix + extension))
        return lines
...
def generate_context(config_path='aitree_config.yaml', dry_run=False):
    ...
    for root, dirs, files in os.walk(initial_folder):
        ...
        for file in files:
            ...
            if ext in extension_whitelist:
                ...
                if file_relpath in filepath_blacklist:
                    continue 

                if filepath_whitelist:
                    file_dir = file_relpath.rsplit('/', 1)[0] if '/' in file_relpath else ""
                    if file_dir in whitelisted_file_dirs:
                        if file_relpath not in filepath_whitelist:
                            continue
    ...
    if not dry_run:
        with open(output_file, 'w', encoding='utf-8') as out_f:
            ...
            # 1. Generate Header and AI Instructions with visible signature
            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            
            out_f.write("# 🌲 AITree Context Document\n")
            out_f.write(f"> **Generated by AITree by n0-se** on `{now}`  \n")
            out_f.write(f"> **Total files included:** `{len(processed_files_data)}`\n\n")
            
            out_f.write("The following document contains the file structure and source code for a project. Please use this context to answer subsequent questions.\n\n")
            out_f.write("---\n\n")
            
            # 2. Inject Optional Project Meta
            if project_title or project_info:
                out_f.write("### Project Context\n")
                if project_title:
                    out_f.write(f"**Project Title:** {project_title}\n\n")
                if project_info:
                    out_f.write(f"**Description:**\n{project_info}\n\n")
                out_f.write("---\n\n")

            # 3. Generate and write the File Tree
            if include_tree and processed_paths:
                out_f.write("### Project File Tree\n```text\n")
                out_f.write(generate_tree_string(processed_paths))
                out_f.write("\n```\n\n")
                out_f.write("---\n\n")

            # 4. Write the actual Source Code blocks
            for file_relpath, content in processed_files_data:
                out_f.write(f"### File: `{file_relpath}`\n```\n{content}\n```\n\n")
